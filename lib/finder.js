...
Finder.prototype.versionRegistry = function (vKey, vValue, pathKey, pathValue, _useWoW) {
  ...
  HIVES.forEach(hive => {
    registry.value(hive, ['Software', vKey], vValue, (err, out, key) => {
      // TODO (later): only ignore "not found" error
      if (err) return next()
  ...
  if (bin) {
    ...
    if (this.ignored.has(id)) {
      ...
      if (metadata) {
        ...
        for (const k in metadata) {
          ...
          if (b[k] != null && b[k] !== metadata[k]) {
            ...
            const v2 = JSON.stringify(metadata[k])
      ...
      if (b) {
        ...
        return mt(bin, function (err, arch) {
          if (err) debug(err)
          else b.arch = arch
          cb()
        })
