...
exports.phantomjs = {
  ...
  find: function() {
    ...
    // 2.x
    try { var path = require('phantomjs-prebuilt').path } catch (err) { }
    if (path) return this.found(path, 'require', this.plan(1))

    // 1.x
    try { var path2 = require('phantomjs').path } catch (err) { }
    if (path2) return this.found(path2, 'require', this.plan(1))
  },
  ...
  pre: function (file, done, _noRecursion) {
    ...
    existent(local, function(err) {
      if (!err) return getBinary(local)

      var global = path.resolve(file, '..', mod)

      existent(global, function(err) {
        if (!err) return getBinary(global)
